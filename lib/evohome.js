...
function mapCollection(items, iteratee) {
...
function getResponseBody(response, body) {
  if (body !== undefined) {
    return body;
  }
...
function formatResponseError(context, response, body) {
  var statusCode = response && response.statusCode;
  var contentType = response && response.headers && response.headers["content-type"];
  var rawBody = getResponseBody(response, body);
  var snippet = "";

  if (rawBody != null) {
    snippet = String(rawBody).replace(/\s+/g, " ").trim().slice(0, 200);
  }

  var parts = [context + " failed"];
  if (statusCode != null) {
...
function parseJsonResponse(context, response, body) {
  var rawBody = getResponseBody(response, body);

  try {
    return JSON.parse(rawBody);
  } catch (e) {
    throw formatResponseError(context, response, rawBody);
  }
}

function shouldRetry(response, body) {
  if (!response || RETRYABLE_STATUS_CODES.indexOf(response.statusCode) === -1) {
    return false;
  }

  var rawBody = getResponseBody(response, body);
  return /service\s+is\s+unavailable/i.test(String(rawBody || ""));
}
...
function Location(json) {
  ...
  this.devices = mapCollection(json.devices, function (device) {
...
function Schedule(json) {
  ...
  this.switchpoints = mapCollection(json.switchpoints, function (sw) {
...
Session.prototype.getSchedule = function (zoneId, isHotWater) {
  ...
  return this._request(url).then(function (json) {
    return mapCollection(json.dailySchedules, function (s) {
...
Session.prototype.getLocations = function () {
  ...
  return this._request(url).then(function (json) {
    return mapCollection(json, function (location) {
      var data = {};
  ...
  request(
    ...
    function (err, response, body) {
      if (err) {
        ...
        try {
          deferred.resolve(parseJsonResponse("set temperature", response, body));
        } catch (e) {
          deferred.reject(e);
        }
      }
  ...
  request(
    ...
    function (err, response, body) {
      if (err) {
        ...
        try {
          deferred.resolve(parseJsonResponse("set system mode", response, body));
        } catch (e) {
          deferred.reject(e);
        }
      }
  ...
  function renew(attempt) {
    request(
      ...
      function (err, response, body) {
        if (err) {
          deferred.reject(err);
        } else if (attempt < 2 && shouldRetry(response, body)) {
          setTimeout(function () {
            renew(attempt + 1);
          }, 1000 * (attempt + 1));
        } else {
          try {
            deferred.resolve(parseJsonResponse("refresh token", response, body));
          } catch (e) {
            deferred.reject(e);
          }
        }
  ...
  request(
    ...
    function (err, response, body) {
      if (err) {
        ...
        try {
          deferred.resolve(parseJsonResponse("GET " + url, response, body));
        } catch (e) {
          deferred.reject(e);
        }
      }
  ...
  function loginAttempt(attempt) {
    request(
      ...
      function (err, response, body) {
        ...
        if (attempt < 2 && shouldRetry(response, body)) {
          setTimeout(function () {
        ...
        try {
          var json = parseJsonResponse("login", response, body);

          if (json.error != null) {
            deferred.reject(new Error("login failed: " + json.error));
          } else if (json.access_token == null) {
            deferred.reject(
              new Error("login failed: No 'access_token' in " + JSON.stringify(json))
            );
          } else {
  ...
  request(
    ...
    function (err, response, body) {
      if (err) {
        ...
        try {
          deferred.resolve(parseJsonResponse("get user account", response, body));
        } catch (e) {
          deferred.reject(e);
        }
      }
